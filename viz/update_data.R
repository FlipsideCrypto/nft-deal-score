...
# swap_to <- transaction_detail[ tx_type == 'Swap' & is.na(to_label), list( contract_address, date, to_address) ]
# swap_fr <- transaction_detail[ tx_type == 'Swap' & is.na(from_label), list( contract_address, date, from_address) ]
# colnames(swap_to)[3] <- 'address'
# colnames(swap_fr)[3] <- 'address'
# swap_fr[, did_swap := 1]
# swap_to <- merge( swap_to, swap_fr, all.x=T, by=c('contract_address','date','address') )
# swap_to[is.na(did_swap), did_swap := 0]
...
transaction_detail <- merge(transaction_detail, price, all.x=T, by=c('contract_address','date'))
...
# determine which are stablecoins
...
library(caret)
dt$pred = predict(rf, data=dt)


############################################
#     Prepare Data Table for Shiny App     #
############################################
publish <- dt
pred_cols <- rownames(rf$importance)
...
for (p in c(pred_cols, 'log_market_cap','log_price')) {
	col.s <- paste0(p, '_std')
...
# show distribution of ballances
...
# calculate LP-adjusted holdings
...
# show where the top holders are putting their tokens
tmp_tx <- merge(tx, cur_ownership[, list(contract_address, address)])
holders <- tmp_tx[ tx_type == 'LP' & !is.na(other_label) ] %>% group_by(contract_address, address, other_label) %>% summarize(net_amt=sum(net_amt)) %>% as.data.table()
holders <- holders[ net_amt < 0 ]
holders[, amount:=-net_amt]

holders <- rbind(
	wallet_amt[, list(contract_address, address, other_label, amount)]
	, holders[, list(contract_address, address, other_label, amount)]
)
holders[, tot:=sum(amount), by=list(contract_address, address)]
holders[, pct:=amount / tot]
colnames(holders)[3] <- 'label'
head(holders)

top_holders <- merge(holders, cur_ownership[, list(contract_address, address)])

load("feature_importance.RData")
farm_data <- farm_data[ !is.na(price_volatility_30d) ]
save(farm_data, cur_date, feature_importance, cur_balance, top_holders, file = "~/data_science/viz/token_tracker/farm_data.RData")
